...
export class MyRacesComponent implements OnInit, OnDestroy {
  ...
  atualUrl: string;
  constructor(
    protected api: TransactionsService,
    protected rapi: RacesService
  ) {}
  ...
  getMyRacesTransactions() {
    this.transObserver = this.api
      .transactionsRacesList(this.actualPage)
      .subscribe((data) => {
  ...
  getRaceWinner(rid: string) {
    this.winnerObservable = this.rapi
      .racesWinnerList(rid, this.actualPageWinner)
      .subscribe((data) => {
